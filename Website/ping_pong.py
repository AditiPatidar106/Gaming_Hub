...
def draw(canvas):
    ...
    if ball_pos[0] <= BALL_RADIUS + PAD_WIDTH and int(ball_pos[1]) in range(int(paddle1_pos[1] - HALF_PAD_HEIGHT),int(
    ...
    if ball_pos[0] >= WIDTH + 1 - BALL_RADIUS - PAD_WIDTH and int(ball_pos[1]) in range(int(
            paddle2_pos[1] - HALF_PAD_HEIGHT), int(paddle2_pos[1] + HALF_PAD_HEIGHT), 1):
        ball_vel[0] = -ball_vel[0]
        ball_vel[0] *= 1
        ball_vel[1] *= 1
